...
const html=readFileSync(require('node:path').join(__dirname,'../password-manager.html'),'utf8');
const script=html.match(/<script>([\s\S]*?)<\/script>/)[1].split('const input = document')[0];
const context=vm.createContext({crypto:webcrypto,TextEncoder,TextDecoder,URL,Uint8Array,DataView,ArrayBuffer});
vm.runInContext(script,context);
test('opens CLI ciphertext and plain text, rejects bad input, and clears or cancels pending results', async () => {
 const nodes=new Map();
 const node=()=>({value:'',checked:false,children:[],textContent:'',handlers:{},setAttribute(){},focus(){},addEventListener(name,fn){this.handlers[name]=fn;},append(...items){this.children.push(...items);},appendChild(item){this.children.push(item);},replaceChildren(){this.children=[];}});
 const get=id=>{if(!nodes.has(id))nodes.set(id,node());return nodes.get(id);};
 get('mode-encrypted').checked=true;
 const ui=vm.createContext({crypto:webcrypto,TextEncoder,TextDecoder,URL,Uint8Array,DataView,ArrayBuffer,atob,document:{getElementById:get,createElement:node},window:{addEventListener(){}},setInterval,clearInterval});
 for(const match of html.matchAll(/<script>([\s\S]*?)<\/script>/g))vm.runInContext(match[1],ui);
 const submit=()=>get('vault-form').handlers.submit({preventDefault(){}});
 const {encryptBytes}=require('../secure-file-encryption.js');
 const encrypted=(await encryptBytes('secret',Buffer.from('Email: pass:word\nsecond'))).toString('base64');
 get('input').value=encrypted;get('password').value='wrong';await submit();
 assert.match(get('status').textContent,/Incorrect password/);assert.equal(get('entries').hidden,true);
 get('password').value='secret';await submit();
 assert.equal(get('entries').hidden,false);assert.equal(get('vault-form').hidden,true);
 assert.equal(get('text-line-cards').children.length,2);assert.equal(get('input').value,'');assert.equal(get('password').value,'');
 get('lock').handlers.click();assert.equal(get('entries').hidden,true);assert.equal(get('text-line-cards').children.length,0);
 get('input').value=encrypted;get('password').value='secret';const pending=submit();get('clear').handlers.click();await pending;
 assert.equal(get('entries').hidden,true);
 get('input').value='not base64!';get('password').value='secret';await submit();assert.match(get('status').textContent,/Base64/);
 get('input').value=(await encryptBytes('secret',Buffer.from([255]))).toString('base64');await submit();assert.match(get('status').textContent,/UTF-8/);
 get('mode-encrypted').checked=false;get('mode-plain').checked=true;get('mode-plain').handlers.change();
 assert.equal(get('password').required,false);assert.equal(get('password-panel').hidden,true);
 get('input').value='<script>alert(1)</script>:  secret  ';await submit();
 const card=get('text-line-cards').children[0];assert.equal(card.children[0].textContent,'<script>alert(1)</script>');
 card.children[2].children[0].handlers.click();assert.equal(card.children[1].textContent,'  secret  ');
});
...
test('cards are separate, update across boundaries, and stop and clear on disposal',async()=>{
 ...
 const ui=vm.createContext({crypto:webcrypto,TextEncoder,TextDecoder,URL,Uint8Array,DataView,ArrayBuffer,Date:{now:()=>now},document:{createElement:node},navigator:{clipboard:{async writeText(value){copied=value;}}},setInterval(fn){tick=fn;return 1;},clearInterval(){cleared=true;}});
...
test('regular secret cards start masked, reveal independently and copy original values', async () => {
 const node=()=>({children:[],textContent:'',handlers:{},setAttribute(){},addEventListener(name,handler){this.handlers[name]=handler;},append(...items){this.children.push(...items);},appendChild(item){this.children.push(item);},replaceChildren(){this.children=[];}});
 let copied;
 const ui=vm.createContext({crypto:webcrypto,TextEncoder,TextDecoder,URL,document:{createElement:node},navigator:{clipboard:{async writeText(value){copied=value;}}}});
 vm.runInContext(script,ui);
